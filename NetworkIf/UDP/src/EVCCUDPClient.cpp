...
bool EVCCUdpClient::sendMulticast(const void* buffer, size_t size)
{
  ...
  SPDLOG_DEBUG("[SUT_IF][ISO15118][UDP]: send multicast: {0} {1} | {2} | size {3}",this->endpoint().address().to_string(), this->endpoint().port(), data_array, size);
...
bool EVCCUdpClient::start()
{
  this->_stop = false;
  if (this->Connect()){
    SPDLOG_DEBUG("[SUT_IF][ISO15118][UDP]: UDP client connected");
    return true;
  }
  else{
    SPDLOG_DEBUG("[SUT_IF][ISO15118][UDP]: UDP client connect failed");
    return false;
  }
}
...
{
  SPDLOG_DEBUG("[SUT_IF][ISO15118][UDP]: on error {0}, {1}, {2}", error, category, message);
...
{
  SPDLOG_DEBUG("[SUT_IF][ISO15118][UDP]: {0} - {1} on sent {2} byte", endpoint.address().to_string(), endpoint.port(), sent);
  if (this->_stop == false){
    this->ReceiveAsync();
  }
...
void EVCCUdpClient::onReceived(const asio::ip::udp::endpoint& endpoint, const void* buffer, size_t size)
{
  ...
  SPDLOG_DEBUG("[SUT_IF][ISO15118][UDP]: on receive {0} {1}| {2} | {3}", addr, endpoint.port(), data_array, size);
...
{
  SPDLOG_DEBUG("[SUT_IF][ISO15118][UDP]: on disconnect");
...
{
  SPDLOG_DEBUG("[SUT_IF][ISO15118][UDP]: start listening on {0} {1}", this->socket().local_endpoint().address().to_string(), this->socket().local_endpoint().port());
  SPDLOG_DEBUG("[SUT_IF][ISO15118][UDP]: multicast msg on {0} {1}", this->endpoint().address().to_string(), this->endpoint().port());
