...
PATimer::PATimer(const std::string &_id, float timeout_value, const std::shared_ptr<CppCommon::Asio::Service> &service) : CppCommon::Asio::Timer(service)
{
  ...
  this->id = _id;
  SPDLOG_DEBUG("[TIMER]: {0} CREATED", this->id);
}

PATimer::~PATimer()
{
  SPDLOG_DEBUG("[TIMER]: {0} DELETED", this->id);
}
...
void PATimer::start()
{
  if (this->state != enPATimerState::EXECUTE)
  {
    ...
    this->tmOutFlag = false;
    this->WaitAsync();
    SPDLOG_DEBUG("[TIMER]: {} START", this->id);
  }
}
...
void PATimer::start(float value)
{
  if (this->state != enPATimerState::EXECUTE)
  {
    ...
    this->tmOutFlag = false;
    this->WaitAsync();
    SPDLOG_DEBUG("[TIMER]: {0} START", this->id);
  }
}
...
void PATimer::stop()
{
  ...
  // }
  // this->tmOutFlag = false;
  SPDLOG_DEBUG("[TIMER]: {0} STOP", this->id);
  this->_clean();
}
...
bool PATimer::wait_timeout()
{
  ...
  this->_clean();
  SPDLOG_DEBUG("[TIMER]: WAIT {} TIMEOUT COMPLETED", this->id);
  return temp;
}
...
void PATimer::onTimer(bool canceled)
{
  if (!canceled)
  {
    ...
    SPDLOG_DEBUG("[TIMER]: {} TIMEOUT", this->id);
  }
  else
  {
    // cancelled
    this->tmOutFlag = false;
    this->state = enPATimerState::COMPLETED;
    SPDLOG_DEBUG("[TIMER]: {} CANCELED", this->id);
  }
}