...
#include "asiowrapper/ssl_client.h"
...
public:
    ...
    void setAddressAndPort(std::string SUTAddress, int SUTPort)
    {
        _address.assign(SUTAddress);
        _port = SUTPort;
    }
        ...
        auto connect_handler = make_alloc_handler(_connect_storage, [this, self]()
        {
            ...
            auto async_connect_handler = make_alloc_handler(_connect_storage, [this, self](std::error_code ec1)
            {
                ...
                if (!ec1)
                {
                    ...
                    SSL_set_tlsext_host_name(_stream.native_handle(), "vortex.data.microsoft.com");
                    // SSL_CTX_set_options
                    if (_strand_required)
...
protected:
    ...
    void onLogging(const std::string& message) { if (_client) _client->onLogging(message); }
...
void SSLClient::setAddressAndPort(std::string addr, int port)
{
    _pimpl->setAddressAndPort(addr, port);
}
