...
int main(int argc, char **argv) {
    ...
    SPDLOG_INFO("**************** START TLS Client DEMO APP **************");
    SPDLOG_INFO("Release date: {}, release time {}", __DATE__, __TIME__);

    auto runenv = std::make_shared<IfRuntime>();
    auto client = std::make_shared<TLSClient>(runenv, IPV6_ADDR, IPV6_PORT, "lo");
    client->start();

   // Connect the client
    std::cout << "Client connecting..." << std::endl;
    client->_pTLSIf->connect();
    ...
    while (getline(std::cin, line))
    {
        ...
        if (line == "!")
        {
            ...
            if (client->_pTLSIf->IsConnected())
                client->_pTLSIf->ReceiveAsync();
            else
                client->_pTLSIf->ConnectAsync();      
        ...
        client->_pTLSIf->SendAsync(line);
    }

    // Disconnect the client
    std::cout << "Client disconnecting..." << std::endl;
    client->_pTLSIf->DisconnectAndStop();
    std::cout << "Done!" << std::endl;
